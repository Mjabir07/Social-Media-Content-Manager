# Product Vision and Principles

## Vision

AZMIN Digital OS is a simple, AI-assisted command center for operating AZMIN
Digital. It connects companies, partnerships, clients, sales, projects, services,
finance, marketing, development, infrastructure and renewals in one platform.

The long-term product may be offered to freelancers, agencies and businesses as a
multi-tenant SaaS product.

## Primary user

The first user is the founder of **AZMIN Digital**, currently working as a solo
operator and performing sales, marketing, project delivery, administration and
support.

The platform must reduce repetitive work without removing human control over
financial, legal, production and client-facing decisions.

## Business contexts

AZMIN may operate as:

- Its own digital-services brand
- An agency serving a client
- A marketing contractor acting inside another company
- A commission-based lead-generation or sales partner
- A joint-venture partner
- The vendor of AZMIN Pulse, a monitoring SaaS product

Each company must have isolated data, integrations, branding, knowledge and agent
instructions.

## Product principles

1. **Simple first:** every screen solves a clear operational problem.
2. **One source of truth:** avoid duplicate records across disconnected tools.
3. **Company isolation:** one company's data must never appear in another context.
4. **Human approval:** important external or destructive actions require approval.
5. **Automation with evidence:** agent work includes logs, results and verification.
6. **Progressive disclosure:** basic fields first; advanced settings only when needed.
7. **Cost awareness:** track provider, AI and infrastructure costs by company/project.
8. **Open-source preference:** prefer reliable open components when they reduce total cost.
9. **Provider independence:** use adapters so one vendor can be replaced later.
10. **SaaS-ready, not SaaS-heavy:** preserve tenant architecture without building
    unnecessary commercial complexity now.

## Automation levels

| Level | Meaning | Examples |
| --- | --- | --- |
| Automatic | Internal and reversible | Tagging, summaries, draft tasks |
| Approval required | External or commercially important | Email, WhatsApp, publishing, invoices, production deployment |
| Manual only | High-risk or irreversible | Payments, contracts, secret export, destructive production operations |

## Out of scope for the early phases

- Fully autonomous financial or legal decisions
- Unofficial social-media or WhatsApp browser automation
- Storing raw secrets in the AZMIN application database
- Building replacements for GitHub, Vercel or a dedicated secrets manager
- Implementing every provider integration simultaneously
- Allowing an agent unrestricted production access

## Success criteria

AZMIN OS succeeds when the owner can open one dashboard and understand:

- What requires attention today
- Which leads and projects are progressing or blocked
- Which payments, commissions and renewals are outstanding
- Which content, deployments or automations require approval
- The revenue, cost and profit of every company and service
- What each AI agent did and how its output was verified

