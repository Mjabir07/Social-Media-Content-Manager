...
import { runRenewalReminders } from "@/lib/renewals";
...
async function run(req: Request) {
  ...
  const [posts, renewals] = await Promise.all([runDuePosts(), runRenewalReminders()]);
  return Response.json({ ok: true, ...posts, renewals });
}
