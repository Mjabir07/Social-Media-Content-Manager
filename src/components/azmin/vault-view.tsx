...
import { KeyRound, Plus, Loader2, X, AlertTriangle, Eye, EyeOff, Copy, Check, Trash2, ExternalLink, ShieldCheck } from "lucide-react";
...
export function VaultView({
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    setLabel(""); setUsername(""); setUrl(""); setSecret(""); setNotes(""); setShowForm(false);
    await refresh();
  }
    ...
    <>
        ...
        {showForm && (
          <form onSubmit={handleCreate} className="mt-6 grid gap-3 rounded-2xl border border-[#C5D6E6] bg-white p-5 shadow-[0_10px_28px_rgba(3,20,46,.05)] sm:grid-cols-2">
            ...
            <label className="text-xs font-bold text-[#476987]">
              Password / secret
              <input className={`mt-1 ${inputCls}`} type="password" value={secret} onChange={(e) => setSecret(e.target.value)} placeholder="stored encrypted" autoComplete="new-password" />
            </label>
            <label className="text-xs font-bold text-[#476987]">
              Panel URL
            ...
            <div className="flex items-center gap-2 sm:col-span-2">
              <button type="submit" disabled={saving} className="inline-flex items-center gap-2 rounded-xl bg-[#087CFA] px-5 py-2.5 text-sm font-bold text-white transition hover:bg-[#076BE0] disabled:opacity-60">
                {saving && <Loader2 size={15} className="animate-spin" />} Save credential
              </button>
                    ...
                    return (
                      <li key={c.id} className="rounded-xl border border-[#D7E4F0] bg-white p-4 shadow-[0_6px_18px_rgba(3,20,46,.04)]">
                        ...
                        <div className="mt-3 flex flex-wrap items-center gap-2 rounded-lg bg-[#F7FAFE] px-3 py-2">
                          ...
                          {canManage && (
                            <button onClick={() => remove(c.id)} disabled={busyId === c.id} className="ml-auto inline-flex items-center rounded-md border border-[#F0C9C4] bg-white px-2 py-1 text-[#C0362C] hover:bg-[#FDECEC] disabled:opacity-60"><Trash2 size={12} /></button>
                          )}
                        </div>
