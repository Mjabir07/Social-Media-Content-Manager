        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h1 className="mt-1 flex items-center gap-3 font-display text-3xl font-bold tracking-[-.03em]">
              <Server className="text-[#087CFA]" size={28} />
              Coolify & Servers
            </h1>
        ...
        <section className="mt-9">
          ...
          {records.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-[#AFC6DE] bg-[#F5F9FE] px-5 py-12 text-center">
              ...
              <h3 className="mt-4 text-base font-bold text-[#173A5C]">No servers connected</h3>
              <p className="mt-1 text-sm text-[#526F8A]">Connect a Hetzner VPS running Coolify to manage your deployments.</p>
              <button className="mt-6 rounded-xl bg-[#087CFA] px-6 py-2.5 text-sm font-bold text-white shadow-sm transition hover:bg-[#076BE0]">
                  ...
                  <div>
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-3">
                        <div className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[#EAF9FF] text-[#087CFA]">
                          <Server size={20} />
                        </div>
                        <div>
                          <div className="font-bold text-[#173A5C]">{record.provider}</div>
                          <div className="text-xs font-semibold text-[#526F8A]">{record.planDetails || "Standard Server"}</div>
                        </div>
                      </div>
                    ...
                    <div className="mt-5 space-y-2">
                      ...
                      {record.renewalDate && (
                        <div className="flex items-center justify-between text-xs">
                          <span className="font-semibold text-[#72899E]">Renewal Date</span>
                          <span className="font-bold text-[#03142E]">{new Date(record.renewalDate).toLocaleDateString()}</span>
                        </div>
                      )}
                    </div>
                  </div>
